...
<div class="header-actions" style="margin-bottom:24px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
    <a href="/admin/passeios/<?= (int) $trip['id'] ?>/horarios/hotel/criar" class="btn btn-primary">+ Adicionar Hotel</a>
    <a href="/admin/passeios/<?= (int) $trip['id'] ?>/horarios/importar" class="btn btn-outline">
    ...
    <?php if (!empty($hotels)): ?>
    <form method="POST" action="/admin/passeios/<?= (int) $trip['id'] ?>/horarios/limpar" class="inline-form" style="margin-left:auto;" onsubmit="return confirm('Tem certeza? Isso removerá TODOS os hotéis e horários deste passeio.')">
        <?= csrf_field() ?>
...
<div class="admin-card" style="text-align:center;padding:60px 20px;">
    ...
    <div style="display:flex;gap:12px;justify-content:center;">
        <a href="/admin/passeios/<?= (int) $trip['id'] ?>/horarios/hotel/criar" class="btn btn-primary">+ Adicionar Hotel</a>
        <a href="/admin/passeios/<?= (int) $trip['id'] ?>/horarios/importar" class="btn btn-outline">Importar Planilha</a>
    </div>
</div>
<?php else: ?>
    ...
    <div class="admin-card schedule-hotel-card <?= $hotel['is_active'] ? '' : 'schedule-hotel-card--inactive' ?>">
        <div class="schedule-hotel-card__header">
            ...
            <div class="actions-cell">
                <a href="/admin/passeios/<?= (int) $trip['id'] ?>/horarios/hotel/<?= (int) $hotel['id'] ?>/editar" class="btn btn-sm btn-outline">Editar</a>
                <form method="POST" action="/admin/passeios/<?= (int) $trip['id'] ?>/horarios/hotel/<?= (int) $hotel['id'] ?>/excluir" class="inline-form" onsubmit="return confirm('Excluir hotel e todos os seus horários?')">
                    <?= csrf_field() ?>
...
<style>
.schedules-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 16px; }
.schedule-hotel-card { padding: 0 !important; overflow: hidden; }
.schedule-hotel-card--inactive { opacity: 0.5; }
.schedule-hotel-card__header { display: flex; align-items: center; justify-content: space-between; padding: 14px 18px; border-bottom: 1px solid var(--border, #e2e8f0); gap: 10px; }
.schedule-hotel-card__name { display: flex; align-items: center; gap: 8px; font-size: 14px; min-width: 0; }
.schedule-hotel-card__name strong { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.schedule-hotel-card__body { padding: 16px 18px; }
.schedule-hotel-card__count { display: block; margin-top: 10px; font-size: 12px; color: #94a3b8; }
.schedule-chips { display: flex; flex-wrap: wrap; gap: 8px; }
.schedule-chip { display: inline-flex; align-items: center; padding: 6px 14px; background: rgba(14,165,233,0.08); color: var(--primary, #0ea5e9); border: 1px solid rgba(14,165,233,0.18); border-radius: 20px; font-size: 13px; font-weight: 700; font-family: 'JetBrains Mono','Fira Code','Consolas',monospace; letter-spacing: 0.5px; }
.schedule-chip--inactive { background: #f1f5f9; color: #94a3b8; border-color: #e2e8f0; text-decoration: line-through; }
</style>
