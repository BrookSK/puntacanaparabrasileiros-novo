-- =============================================================
-- ATUALIZAÇÃO DE CONTEÚDO: Buggies e Quadriciclos
-- Preenche apenas campos de TEXTO. NÃO toca em nenhum preço/pacote.
--
-- COMO USAR:
-- 1. FAÇA BACKUP DO BANCO ANTES (phpMyAdmin > Exportar).
-- 2. Rode o SELECT [A] para descobrir o id do passeio.
-- 3. No passo [B], troque o id do WHERE pelo id correto e rode.
-- =============================================================

-- [A] DESCUBRA O ID (não altera nada). Procura por buggy/quadriciclo/aventura.
SELECT id, title FROM trips
WHERE title LIKE '%uggy%' OR title LIKE '%uggie%'
   OR title LIKE '%uadricicl%' OR title LIKE '%dventure%' OR title LIKE '%acao%'
ORDER BY id;
...
WHERE id = 0;  -- ??  TROQUE pelo id do "Buggies e Quadriciclos"
